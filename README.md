...
```bash
aws lambda create-function --function-name my-app-elasticsearch \
...
```bash
aws lambda invoke --function-name my-app-elasticsearch --payload '{"resource": "/{proxy+}", "path": "/ping", "httpMethod": "GET"}' build/response.txt
